...
function TasksList({ tasks, members, labels, board, group, openModal, onDeleteTask, isCollapsed }) {
    ...
    useEffect(() => {
        socketService.on('task-changed', (updatedTask) => {
            setTaskList(prevTaskList => prevTaskList.map(task => task._id === updatedTask._id ? updatedTask : task));
            showSuccessMsg('Task updated successfully')
        });

        socketService.on('task-added', (newTask) => {
            setTaskList(prevTaskList => [...prevTaskList, newTask]);
        });

        socketService.on('task-removed', ( removedTask ) => {
            setTaskList(prevTaskList => prevTaskList.filter(task => task._id !== removedTask._id));
        });
        ...
        return () => {
            socketService.off('task-changed');
            socketService.off('task-removed');
